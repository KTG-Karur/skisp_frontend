...
import { routes, dontCkeckRouts } from '../../router/routes';
...
const DefaultLayout = ({ children }: PropsWithChildren) => {
    ...
    const userRout: string[] = localData?.pagePermission || [];

    const location = useLocation();

    const extractedPaths = useMemo(() => {
        return userRout?.filter((r: any) => r && r?.url).map((r: any) => r?.url) || [];
    }, [userRout]);

    const currentPath = location.pathname;

    const isAllowed = dontCkeckRouts.some((r) => matchPath({ path: r.path, end: true }, currentPath)) || extractedPaths.some((p) => matchPath({ path: p, end: true }, currentPath));
            ...
            <div className="relative">
                ...
                {showLoader && (
                    <div className="screen_loader fixed inset-0 bg-[#fafafa] dark:bg-[#060818] z-[60] grid place-content-center animate__animated">
                        <svg width="64" height="64" viewBox="0 0 135 135" xmlns="http://www.w3.org/2000/svg" fill="#ef7f1b">
