...
const LivePaymentTest = () => {
    ...
    const parseExpiryDate = (apiResponse) => {
        try {
            ...
            if (apiResponse.userDetails?.results) {
                const expiryField = apiResponse.userDetails.results.find((item) => item.fid === 'expire_time');
    ...
    // Function to check if expiry date is within 10 days
    const checkExpiryDate = (expiryData) => {
        try {
            if (!expiryData) {
                console.log('No expiry date data provided');
                // If no expiry date, allow payment as fallback
            ...
            // Check if within 10 days (including expired accounts)
            ...
            // Set message based on days remaining
            let message = '';
            if (daysRemaining < 0) {
            ...
            // If there's an error parsing date, allow payment as fallback
            setCanMakePayment(true);
    ...
    const fetchUserData = async (id) => {
        try {
            ...
            if (response.data?.data?.success) {
                ...
                // Check expiry date
                checkExpiryDate(expiryData);

                setPaymentData({
                    userId: userDetails.user_id || id,
                    amount: parseFloat(userDetails.total_price) || 590,
                    paymentFor: 'internet_recharge',
                    serviceType: 'bandwidth_plan',
            ...
            // If error, allow payment as fallback
            setCanMakePayment(true);
        ...
        <>
            ...
            >
                <div className="p-4">
                    ...
                    <h3 className="text-lg font-semibold text-center text-gray-800 mb-3">{canMakePayment ? 'Payment Available' : 'Payment Window Not Open'}</h3>

                    <div className={`p-4 rounded-lg mb-4 ${getExpiryStatusClass()}`}>
                        <div className="space-y-2">
                            <p className="text-sm text-gray-800">
                                <strong>Account Expiry:</strong> {expiryInfo.formattedDate || expiryInfo.expiryString || 'Not Available'}
                            </p>
                            <p className="text-sm text-gray-800">
                            ...
                            <p className="text-sm text-gray-800">
                                <strong>Expiry Source:</strong> {expiryInfo.expiryString ? 'API String' : expiryInfo.expiryDate ? 'Unix Timestamp' : 'Not Available'}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        <p className="text-gray-600 text-sm">
                            <strong>Payment Policy:</strong> You can make payments only when your account expiry is within 10 days or has already expired.
                        </p>

                        {canMakePayment ? (
                            <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                                <p className="text-sm text-green-700">✅ You are eligible to make a payment. Your account expiry falls within the payment window.</p>
                            </div>
                        ) : (
                            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                                <p className="text-sm text-yellow-700">
                                    ⏳ Payment will be available from{' '}
                                    {expiryInfo.expiryDate ? new Date(expiryInfo.expiryDate.getTime() - 10 * 24 * 60 * 60 * 1000).toLocaleDateString() : 'when 10 days before expiry'}.
                                </p>
                            </div>
                        )}
            ...
            <div style={styles.container}>
                    ...
                    <div>
                            ...
                            {expiryMessage && (
                                <div
                                    style={{
                                        ...
                                        color: expiryInfo.daysRemaining === null ? '#1e40af' : expiryInfo.daysRemaining < 0 ? '#7f1d1d' : expiryInfo.daysRemaining <= 10 ? '#7c2d12' : '#065f46',
                        ...
                        >
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <div>
                                    <div style={{ fontWeight: '700', fontSize: isMobile ? '13px' : '14px' }}>Need Help?</div>
                                    <div
                                        style={{
                ...
                >
                    ...
                    <div style={{ marginTop: '4px', fontSize: isMobile ? '11px' : '12px' }}>
                        Secured by Razorpay • User ID: {userId} • Expiry: {expiryInfo.daysRemaining ?? 'N/A'} days remaining
                    </div>
                </div>
            </div>
        </>
